...
URL = "https://apitotvsmoda.bhan.com.br/api/totvsmoda/financial-panel/v2/ranking-customer-debtors/search"
...
# === SALVA DEBUG ===
debug_file = f"debug_ranking_customer_debtors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
if not df_data.empty:
    rename_map = {
        ...
        "customer_value": "Valor Devedor"
    }
    df_data.rename(columns=rename_map, inplace=True)
    print("📝 Colunas renomeadas para nomes amigáveis.")

# === EXPORTAÇÃO PARA EXCEL ===
excel_file = f"ranking_customer_debtors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
try:
